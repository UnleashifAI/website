...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-black/90 shadow-lg backdrop-blur-md py-3' : 'bg-transparent py-5'
      }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        ...
        >
          <div className="w-6 h-5 flex flex-col justify-between">
            <span className={`block h-0.5 w-full ${scrolled ? 'bg-white' : 'bg-black dark:bg-white'} transform transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-2' : ''
              }`}></span>
            <span className={`block h-0.5 w-full ${scrolled ? 'bg-white' : 'bg-black dark:bg-white'} transition-opacity duration-300 ${menuOpen ? 'opacity-0' : 'opacity-100'
              }`}></span>
            <span className={`block h-0.5 w-full ${scrolled ? 'bg-white' : 'bg-black dark:bg-white'} transform transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-2' : ''
              }`}></span>
          </div>
      ...
      {/* Mobile Menu */}
      <div className={`fixed inset-0 bg-black/95 backdrop-blur-lg z-0 transition-transform transform duration-300 ease-in-out ${menuOpen ? 'translate-x-0' : 'translate-x-full'
